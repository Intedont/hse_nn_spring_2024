...
def parse_args():
    ...
    # Dataset
    parser.add_argument('--dataset', type=str, default='ModelNet40', help='ModelNet40 or ScanObjectNN')
...
def main(args):
    ...
    wandb.init(
            project='repsurf_training',
            name=args.dataset + '_' + str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')) 
            )
    ...
    if(args.dataset == 'ModelNet40'):
        DATA_PATH = os.path.join(args.data_dir, 'modelnet40_ply_hdf5_2048')
        TRAIN_DATASET = ModelNet40(data_dir=DATA_PATH, split='train')
        TEST_DATASET = ModelNet40(data_dir=DATA_PATH, split='test')
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        wandb.log({
                    ...
                    'lr': lr,
                    'train_acc': train_instance_acc * 100
                    })

        if epoch >= args.min_val:
            with torch.no_grad():
                sing_acc, vote_acc = test(classifier.eval(), testDataLoader,
                                          num_class=40 if args.dataset == 'ModelNet40' else 15,
                ...
                wandb.log({
                    'val_single_acc': sing_acc * 100,
                    'val_vote_acc': vote_acc * 100,
                    })
